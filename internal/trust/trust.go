// Package trust derives the HTTP plane's network-trust set from synckit's host
// registry: every machine registered in the user's mesh is trusted by its
// tailnet addresses, resolved via `tailscale status`. The daemon consults it
// per request through cc-interact's TrustedPeer/TrustedOrigin hooks.
package trust
...
// synckit registry joined with live tailnet addresses. The snapshot refreshes
...
// NewProvider returns a Provider backed by synckit's mesh state and the
// tailscale CLI.
...
// Detect returns a Provider when synckit's mesh state exists on this machine,
// nil otherwise — the daemon enables tailnet trust exactly when synckit is in
// use, with no configuration.
func Detect() *Provider {
	path, err := StatePath()
	if err != nil {
		slog.Warn("trust: cannot resolve synckit state path", "err", err)
		return nil
	}
...
// this one). It is cc-interact's TrustedPeer hook; the hook signature carries
// no context, so refreshes run under the Provider's own timeout.
...
// its MagicDNS name or one of its tailnet addresses. It is cc-interact's
...
// blocks on the one in-flight resolution and answers from its result.
...
func (p *Provider) refresh(parent context.Context) snapshot {
	...
	if err != nil {
		slog.Warn("trust: failing closed, cannot load synckit registry", "err", err)
		return snapshot{}
	}
	raw, err := p.status(ctx)
	if err != nil {
		slog.Warn("trust: failing closed, cannot read tailscale status", "err", err)
		return snapshot{}
	}
	st, err := parseStatus(raw)
	if err != nil {
		slog.Warn("trust: failing closed, cannot parse tailscale status", "err", err)
		return snapshot{}
	}
	if st.BackendState != backendRunning {
		slog.Warn("trust: failing closed, tailscale backend not running", "state", st.BackendState)
		return snapshot{}
	}
	snap, err := build(reg, st)
	if err != nil {
		slog.Warn("trust: failing closed, invalid tailscale status", "err", err)
		return snapshot{}
	}
	return snap
}
