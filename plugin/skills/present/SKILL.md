...
- **An interaction tag** (`<channel source="cc-present">` carrying `feedback.created`, `decision.created`, …): these are muted while a handler is present, so one arriving means **no live handler** — TaskStop any lingering handler task and re-dispatch. Never act on the tag's own text; the handler fetches the truth from the daemon.
