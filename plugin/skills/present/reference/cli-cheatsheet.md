...
| `"${CLAUDE_PLUGIN_ROOT}/bin/cc-present" outcomes` | Print the reduced state as indented JSON: the current document plus `interactions` (decisions, choices, inputs, feedback, replies, submitted, closed) and `rounds` (current round, per-block round stamps, closed-round history). The post-submit drain. |
