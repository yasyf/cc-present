...
`revising` is your own declared working set reflected back in state (see Agent-origin events): the top-level block ids you announced you're rewriting, plus an optional shared `note`. It clears itself as you upsert those blocks, so it reads `{ "blockIds": [] }` at rest — it is your announcement, not a human interaction. On the human side, a `choices` entry gains an `other` string when the human wrote in past the options, and `feedback` keys any block, so a choice's note thread lands there beside its selection.
