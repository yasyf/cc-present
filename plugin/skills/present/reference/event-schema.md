...
| `round.started` | `{title?, carry?}` — dirty round: snapshots it into `rounds.history` (no `submittedRevision`), advances, restamps each `carry` id into the new round (the daemon computes carry as the closing round's unanswered top-level blocks), then titles the new round; clean round: only titles the current one, ignoring carry. Either way it clears the revising working set. The revision is untouched; it counts `doc.replaced` alone | `round`, or `push`/`update-block` with `--round new` |
