...
# Locate the binrun runner and exec the cc-present descriptor.
#
# This renders to plugin scripts/install-binary.sh — the successor to the
# provision-a-symlink installer. bin/cc-present is a committed symlink to it, so
...
# resolves and execs the version-exact artifact the sidecar bin/cc-present.binrun
...
if [ -e "$ROOT/.orphaned_at" ] && is_version "${ROOT##*/}"; then
  ...
  for sibling in "${ROOT%/*}"/*/; do
    ...
    [ ! -e "$sibling/.orphaned_at" ] && [ -x "$sibling/bin/cc-present" ] || continue
    if version_gt "${sibling##*/}" "${newest##*/}"; then
      newest=$sibling
    fi
  done
  [ "$newest" = "$ROOT" ] || exec "$newest/bin/cc-present" "$@"
fi

DESCRIPTOR="$ROOT/bin/cc-present.binrun"
